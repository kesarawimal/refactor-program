...
public class PictureFrame {
  ...
  class DominoPanel extends JPanel {
    ...
    protected void paintComponent(Graphics g) {
      ...
      if (master.mode == 1) {
        ...
        drawHeadings(g);
        drawGrid(g);
        drawByMode(g,master.guessesList);
      }
      if (master.mode == 0) {
        drawGridLines(g);
        drawHeadings(g);
        drawGrid(g);
        drawByMode(g,master.dominoList);
      }
    }

    private void drawByMode(Graphics g, List<Domino> dominos) {
  ...
  public PictureFrame(Aardvark sf) {
    master = sf;
